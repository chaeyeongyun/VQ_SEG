...
def train(cfg):
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx in pbar:
            ...
            pseudo_1 = model_1.pseudo_label(easy_ul).clone().detach()
            pseudo_2 = model_2.pseudo_label(easy_ul).clone().detach()
            pseudo_1 = hard_aug(inverse_similarity_transform(pseudo_1, aug, angle), mask=mask)[0]
            pseudo_2 = hard_aug(inverse_similarity_transform(pseudo_2, aug, angle), mask=mask)[0]
...
if __name__ == "__main__":
    ...
    cfg.resize = 448
    # train(cfg)
    cfg.project_name = 'vq_pt_unet_weighted_cross_entropy_easyhard'
