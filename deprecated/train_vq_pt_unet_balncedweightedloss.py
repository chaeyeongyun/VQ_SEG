...
from loss import make_loss_as_func, compute_class_weight
...
def train(cfg):
    ...
    criterion = make_loss_as_func(cfg.train.criterion.name)
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx in pbar:
            ...
            with torch.cuda.amp.autocast(enabled=half):
                cls_weight = compute_class_weight(num_classes, l_target).to(device)
                
                ## cps loss
                cps_loss = criterion(pred_1, pseudo_2, weight=cls_weight) + criterion(pred_2, pseudo_1, weight=cls_weight)
                ## supervised loss
                sup_loss_1 = criterion(pred_sup_1, l_target, weight=cls_weight)
                sup_loss_2 = criterion(pred_sup_2, l_target, weight=cls_weight)
...
if __name__ == "__main__":
    ...
    cfg.resize = 448
    # train(cfg)
    cfg.project_name = 'vq_pt_unet_balnced_weight'
