...
def make_regularized_pseudo_label(raw_pseudo, percent):
    ...
    thresh_mask = entropy.ge(thresh).bool() # thresh보다 큰 entropy인 위치는 True
    pseudo_label[thresh_mask] = 255 # loss연산에서 무시
    return pseudo_label
...
def train(cfg):
    ...
    criterion = make_loss(cfg.train.criterion.name, num_classes, weight=loss_weight, ignore_index=255)
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx in pbar:
            ...
            pseudo_1 = make_regularized_pseudo_label(pred_1, drop_percent)
            pseudo_2 = make_regularized_pseudo_label(pred_2, drop_percent)
