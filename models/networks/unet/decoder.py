...
from models.modules.attention import CCA
import torch
...
class CCAUnetDecoder(nn.Module):
    def __init__(self, encoder_channels, decoder_channels, cca:List):
        super().__init__()
        encoder_channels = encoder_channels[1:] # (64, 256, 512, 1024, 2048)
        encoder_channels = encoder_channels[::-1] # (2048, 1024, 512, 256, 64)
        ...
        prev_channels = 0
        for i in range(n_blocks):
            temp = [double_conv_block(in_channels=encoder_channels[i]+prev_channels, out_channels=decoder_channels[i])]
            if cca[i]:
                temp.append(CCA(decoder_channels[i], decoder_channels[i]))
            else:
