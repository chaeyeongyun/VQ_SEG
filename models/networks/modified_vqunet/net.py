...
##
class VQOnlyPTUnet1x1(nn.Module):
    def __init__(
        self, 
        encoder_name:str,
        num_classes:int,
        ...
        self.decoder = UnetDecoder(encoder_channels,
                                decoder_channels)
        self.segmentation_head = nn.Conv2d(decoder_channels[-1], num_classes, 1, bias=False)
        self.prototype_loss = OnlyCWPrototypeLoss(num_classes, decoder_channels[-1], margin=margin, scale=scale, init=pt_init, use_feature=use_feature, )
        self.device = None
        self.upsampling = nn.UpsamplingBilinear2d(scale_factor=upsampling) if upsampling > 1 else nn.Identity()
