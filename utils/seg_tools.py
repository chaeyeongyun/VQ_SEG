...
def onehot_1d(target:torch.Tensor, num_classes:int, eps:float=1e-6):
    """onehot encoding for 1 channel labelmap

    Args:
        target (torch.Tensor): shape (HxW, 1) or (HxW, ) have label values
        num_classes (int): the number of classes
    """
    if target.dim()==1:
        target = target.unsqueeze(-1)
    onehot = torch.zeros((target.shape[0], num_classes), dtype=torch.float64, device=target.device)
    onehot = onehot.scatter_(1, target.long(), 1.0) + eps
    return onehot
