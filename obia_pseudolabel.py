...
def make_feature(image, image_gray, gt_image):
    ...
    return featarray, train_y

def main(image_root="/content/data/semi_sup_data/CWFID/num30/train"):
    target_list = glob(osp.join(image_root, "target", "*.png"))
    feat_list = []
    y_list = []
    for target in tqdm(target_list):
        filename = osp.split(target)[-1]
        gt = target
        ...
        featarray, train_y =  make_feature(image, image_gray, gt_image)
