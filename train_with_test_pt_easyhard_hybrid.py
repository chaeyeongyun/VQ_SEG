...
def train(cfg):
    ...
    for epoch in range(num_epochs):
        ...
        model_2.train()
        ep_start = time.time()
        if epoch < cfg.train.only_sup_epochs:
            ...
            for batch_idx in pbar:
                ...
                with torch.cuda.amp.autocast(enabled=half):
                    ...
                    commitment_loss = commitment_loss_l1 + commitment_loss_l2
                    commitment_loss *= total_commitment_loss_weight
                    
                    prototype_loss = prototype_loss_l1 + prototype_loss_l2
                    prototype_loss *= total_prototype_loss_weight
                    
                    loss = sup_loss  + commitment_loss + prototype_loss
                    sum_code_usage += (code_usage_l1 + code_usage_l2) / 2
                ...
                cps_loss = torch.tensor([0.], device=device, requires_grad=False, dtype=torch.float32 )
                ...
                print_txt = f"[Epoch{epoch}/{cfg.train.num_epochs}][Iter{batch_idx+1}/{len(unsup_loader)}] lr={learning_rate:.5f}" \
                                + f"miou={step_miou:4f}, sup_loss_1={sup_loss_1.item():.4f}, prototype_loss={prototype_loss.item():.4f}, cps_loss={cps_loss.item():.4f}"
            ...
            for batch_idx in pbar:
                ...
                with torch.no_grad():
                    ...
                    pseudo_aug_1 = torch.argmax(model_1(easy_ul)[0], dim=1)
                    pseudo_aug_2 = torch.argmax(model_2(easy_ul)[0], dim=1)
                    pseudo_aug_1 = hard_aug(inverse_similarity_transform(pseudo_1, aug, angle), mask=mask)[0]
                    pseudo_aug_2 = hard_aug(inverse_similarity_transform(pseudo_2, aug, angle), mask=mask)[0]
                    model_1.train()
                    model_2.train()
                ...
                with torch.cuda.amp.autocast(enabled=half):
                    ...
                    pred_aug_ul_1, commitment_loss_aug_ul1, code_usage_aug_ul1, prototype_loss_aug_ul1 = model_1(hard_ul, pseudo_aug_2, percent=drop_percent)
                    pred_aug_ul_2, commitment_loss_aug_ul2, code_usage_aug_ul2, prototype_loss_aug_ul2 = model_2(hard_ul, pseudo_aug_1, percent=drop_percent)
                ...
                # pseudo label
                no_aug_pseudo_1 = torch.argmax(pred_1.detach(), dim=1).long()
                no_aug_pseudo_2 = torch.argmax(pred_2.detach(), dim=1).long()
                pred_1 = torch.cat([pred_1, pred_aug_ul_1])
                pred_2 = torch.cat([pred_2, pred_aug_ul_2])
                
                pseudo_1 = torch.cat([no_aug_pseudo_1, pseudo_aug_1])
                pseudo_2 = torch.cat([no_aug_pseudo_2, pseudo_aug_2])
                ...
                with torch.cuda.amp.autocast(enabled=half):
                    ...
                    sup_loss = sup_loss_1 + sup_loss_2
                    ## commitment_loss
                    commitment_loss = commitment_loss_l1 + commitment_loss_l2 + commitment_loss_ul1 + commitment_loss_ul2 +  commitment_loss_aug_ul1 + commitment_loss_aug_ul2
                    commitment_loss *= total_commitment_loss_weight
                    ## prototype_loss
                    prototype_loss = prototype_loss_l1 + prototype_loss_l2 + prototype_loss_ul1 + prototype_loss_ul2 + prototype_loss_aug_ul1 + prototype_loss_aug_ul2
                    prototype_loss *= total_prototype_loss_weight
                    ...
                    sum_code_usage += (code_usage_l1 + code_usage_l2 + code_usage_ul1 + code_usage_ul2 + code_usage_aug_ul1 + code_usage_aug_ul2) / 6
        ...
        if logger != None:
            ...
            params = [l_input, l_target, pred_sup_1, hard_ul, pred_aug_ul_1] if epoch >= cfg.train.only_sup_epochs else [l_input, l_target, pred_sup_1, None, None]
